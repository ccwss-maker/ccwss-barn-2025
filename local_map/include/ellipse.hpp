...
vector<Ellipse> get_ellipse_array(std::vector<Obstacle> &seg_obs, const grid_map::GridMap &localmap_obs, float localmap_x_size,float localmap_y_size,float resolution, float _inv_resolution)
{
  ...
  for (const auto &obs : seg_obs)
  {
    ...
    for (const auto &pt : ds_pts)
    {
      ...
      Vector2d pos;
      pos(0) = (pt(0) - localmap_x_size / resolution * 0.5) * resolution + localmap_obs.getPosition().x();
      pos(1) = (pt(1) - localmap_y_size / resolution * 0.5) * resolution + localmap_obs.getPosition().y();
    ...
    if (!get_min_area_ellipse(pts, ellipse))
    {
      ...
      ellipse.cx = (grid_center(0) - localmap_x_size / resolution * 0.5) * resolution + localmap_obs.getPosition().x();
      ellipse.cy = (grid_center(1) - localmap_y_size / resolution * 0.5) * resolution + localmap_obs.getPosition().y();
...
void visEllipse(const std::vector<Ellipse> &obs_ellipses, ros::Publisher &_Ellipses_vis_pub)
{
  ...
  shape_vis.header.frame_id = "odom";
