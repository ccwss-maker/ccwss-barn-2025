...
class Local_Planner():
    def __init__(self):
        ...
        self.__sub_goal = rospy.Subscriber('/move_base/TrajectoryPlannerROS/global_plan', Path, self.__global_path_cb, queue_size=20)
    ...
    def __replan_cb(self, event):
        if self.choose_goal_state():
            ...
            for i in range(1, self.N):
                dx = self.goal_state[i, 0] - self.goal_state[i-1, 0]
                dy = self.goal_state[i, 1] - self.goal_state[i-1, 1]
                if dx != 0 or dy != 0:
                    desired_theta = np.arctan2(dy, dx)
                    prev_theta = self.goal_state[i-1, 2]
                    delta = np.arctan2(np.sin(desired_theta - prev_theta), np.cos(desired_theta - prev_theta))
                    self.goal_state[i, 2] = prev_theta + delta
                elif i !=0:
                    self.goal_state[i, 2] = self.goal_state[i-1, 2]
                else:
                    self.goal_state[i,2]=0
            ...
            # for i in range(self.N - 1):
            #     dx = self.goal_state[i+1, 0] - self.goal_state[i, 0]
            #     dy = self.goal_state[i+1, 1] - self.goal_state[i, 1]

            #     if dx != 0.0 or dy != 0.0:
            #         theta = np.arctan2(dy, dx)
            #         # Enforce forward motion: avoid sudden 180° flip
            #         if i > 0:
            #             prev_theta = self.goal_state[i-1, 2]
            #             angle_diff = np.arctan2(np.sin(theta - prev_theta), np.cos(theta - prev_theta))
            #             if np.abs(angle_diff) > np.pi / 2:
            #                 theta += np.pi
            #                 theta = np.arctan2(np.sin(theta), np.cos(theta))  # Normalize to [-π, π]
            #         self.goal_state[i, 2] = theta
            #     else:
            #         self.goal_state[i, 2] = self.goal_state[i-1, 2] if i > 0 else 0.0

            # # Final goal orientation (last point)
            # dx_final = self.goal_state[-1, 0] - self.goal_state[-2, 0]
            # dy_final = self.goal_state[-1, 1] - self.goal_state[-2, 1]
            # self.goal_state[-1, 2] = np.arctan2(dy_final, dx_final)
###above modified
    ...
    def __publish_local_plan(self, input_sol, state_sol):
        ...
        local_path_vis.header.frame_id = "odom"
    ...
    def MPC_ellip(self):
        ...
        def is_obstacle_nearby(ob_):
            ...
            distance = np.linalg.norm(self.curr_state[:2] - cross_pt)
            
            return distance < 3.0

        
        def perpendicular_obs(ob_):
            ob_vec = np.array([ob_[0], ob_[1]])
            vec_to_ob = ob_vec - self.curr_state[:2]

            distance = np.linalg.norm(vec_to_ob)
            if distance > 3.0 or distance < 1e-5:
                return False  # too far or too close (avoid divide by zero)

            heading = self.goal_state[self.N-1, :2] - self.curr_state[:2]
            ...
            cos_theta = np.dot(heading, vec_to_ob)
            return -0.15 < cos_theta < 0.15
        ...
        elif(distance_global(self.curr_state, self.global_path[-1, :2]) > 0.5):
        ...
        for j in range(num_obs):
            if not exceed_ob(self.ob[20*j]) and is_obstacle_nearby(self.ob[20*j]):
                for i in range(self.N-1): 
                        opti.subject_to(h(opt_states[i + 1, :], self.ob[j * 20 + i + 1]) >=
                                    (1 - gamma_k) * h(opt_states[i, :], self.ob[j * 20 + i]))
        ...
        A = np.diag([0.1, 0.02])
        for i in range(self.N):
            Q = np.diag([1.0+0.1*i,1.0+0.1*i, 0.03+0.01*i])
        ...
        #     state_res = self.last_state
            ...
            rospy.logerr("MPC Infeasible: %s", str(e))
            rospy.logwarn("Using fallback trajectory from Simulated Annealing")

            fallback = fallback_trajectory(self.curr_state, self.goal_state[-1], self.N)
            state_res = fallback
            u_res = [np.zeros(2)] * self.N  # dummy controls just to keep types consistent
...
def fallback_trajectory(curr_state, goal_state, N=20):
    def energy(traj):
        total = 0.0
        for i in range(1, N):
            total += np.linalg.norm(traj[i] - goal_state)
            total += np.linalg.norm(traj[i] - traj[i-1]) * 0.2  # smoothness penalty
        return total
    ...
    def neighbor(traj):
        ...
        new_traj[idx] = new_traj[idx] + noise
        return new_traj

    traj = np.linspace(curr_state, goal_state, N)
    best_traj = traj.copy()
    best_energy = energy(traj)
    ...
    while T > T_min:
        new_traj = neighbor(best_traj)
        new_energy = energy(new_traj)

        if new_energy < best_energy or np.random.rand() < np.exp(-(new_energy - best_energy) / T):
            best_traj = new_traj
